/*
  ...
  *
  * Algorithm: 
  * replace all possible values with joker
  * compare hands
...
const PICT_CARD_VALS = { T: 10, J: 11, Q: 12, K: 13, A: 14, J: 1 };
...
const JOKER_REGEX = /J/g
const POSSIBLE_CARDS = 'A23456789TJQK';
...
function getAllPossibleHands(hand) {
  if (!JOKER_REGEX.test(hand)) {
    return [hand];
  }
  return POSSIBLE_CARDS.split('').map(card => hand.replace(JOKER_REGEX, card));
}

function getBestHandValue(possibleHands) {
  return possibleHands.reduce((bestValue, hand) => {
    const value = determineHandValue(hand);
    return value > bestValue ? value : bestValue;
  }, 0);
}

function compareHands(handA, handB) {
  const possibleHandsA = getAllPossibleHands(handA);
  const possibleHandsB = getAllPossibleHands(handB);

  const bestHandValueA = getBestHandValue(possibleHandsA);
  const bestHandValueB = getBestHandValue(possibleHandsB);

  if (bestHandValueA !== bestHandValueB) return bestHandValueB - bestHandValueA;

  // If the best hand values are equal, use the original hands for tie-breaking
